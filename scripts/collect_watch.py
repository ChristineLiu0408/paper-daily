...
def dedupe_records(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        key = f"doi:{doi}" if doi else f"url:{link}" if link else f"title:{title}:{year}"
        if key in seen:
            continue
        seen.add(key)
...
def main(argv: list[str] | None = None) -> int:
    ...
    for watch in selected_watches(config, args.watch):
        ...
        payload = collect_watch(watch, config, args.since_days, now, source_types=source_types, conference_state=state)
